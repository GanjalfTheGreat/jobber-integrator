...
def _find_id_by_sku(
    ...
    """Paginate through productOrServices; return node id where name matches sku, or None. Raises TokenExpiredError on 401."""
    after: str | None = None
    while True:
        variables: dict[str, Any] = {"first": 100, "after": after}
        resp = _graphql_request(session, headers, QUERY_PRODUCTS_PAGE, variables)
...
def run_sync(account_id: str, rows: list[tuple[str, float]]) -> dict[str, Any]:
    """
    Run sync for the given account_id and CSV rows. Uses get_valid_access_token;
    on 401 refreshes and retries that request once.
    Returns {"updated": int, "skus_not_found": list[str], "error": str | None}.
    """
    ...
    result: dict[str, Any] = {"updated": 0, "skus_not_found": [], "error": None}
    ...
    for sku, cost in rows:
        if result["error"]:
            break
        try:
            node_id = _find_id_by_sku(session, headers, sku)
